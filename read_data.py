...
class DataProcessor:
    ...
    def index_data(
        ...
        use_event_structure=True,
        min_event_structure=1,
        max_event_structure=1,
        min_args_event=1,
        return_data=False,
        verbose=False,
    ):
        result = asyncio.run(
            self._async_index_data(
                filename,
                tokenize,
                add_new_words,
                pad_info,
    ...
    async def _async_index_data(
        ...
        count_rows = 0
        BUFFER_ROWS = 1000
        async with async_read_txt_file(
            filename, buffer_hint=BUFFER_HINT, encoding="utf-8", errors="replace", verbose=verbose
        ) as opened_file:
        ...
        for row in rows_batch:
            ...
            for datum_event_structure in list_datum_event_structure:
                if (min_args_event is not None) and (len(datum_event_structure.keys()) < max(min_args_event, 1)):
                    ...
                    continue
                indexed_event_args = {
                    key: self._index_string(datum_event_structure[key], tokenize=tokenize, add_new_words=add_new_words)
    ...
    def _index_string(self, string: str, tokenize=None, add_new_words=True):
        ...
        token_indices = [
            self.word_index[token] if token in self.word_index else self.word_index["UNK"] for token in tokens
        ]
    ...
    def pad_data(self, indexed_data, pad_info, use_event_structure=True, return_data=False):
        ...
        if not use_event_structure:
            ...
            for event_structure in indexed_event_structures:
                ordered_event_structure = [
                    event_structure[arg_type] if arg_type in event_structure else [self.word_index["NONE"]]
            ...
            if "max_arg_length" in pad_info:
                ...
                self.max_arg_length = max(
                    [max([len(arg) for arg in structure]) for structure in ordered_event_structures]
                )

            for event_structure in ordered_event_structures:
                event_inputs.append(
                    csr_matrix(
                        [self._pad_indexed_string(indexed_arg, self.max_arg_length) for indexed_arg in event_structure]
                    )
                )
    ...
    def _pad_indexed_string(self, indexed_string: list[int], max_string_length: int):
        ...
        padded_string = ([self.word_index["NONE"]] * (max_string_length - string_length) + indexed_string)[
            -max_string_length:
        ]
    ...
    def get_embedding(self, embedding_file, add_extra_words=False, verbose=False):
        ...
        embedding_matrix = np.zeros(
            shape_embedding
        )  # np.random.uniform(low_embedding, high_embedding, shape_embedding)
        ...
        LOGGER.info(
            f"NORMALIZED EMBEDDING LOW: {low_embedding.min()}\tNORMALIZED EMBEDDING HIGH: {high_embedding.min()}"
        )
    ...
    async def _async_get_embedding_from_txt(self, embedding_file, verbose=False):
        ...
        async with async_read_txt_file(
            embedding_file, buffer_hint=BUFFER_HINT, encoding="utf-8", errors="replace", verbose=verbose
        ) as opened_file:
...
@contextmanager
def file_txt_buffered(filename: str, buffer_hint: int = -1, encoding="utf-8", errors=None, verbose=False):
...
@asynccontextmanager
async def async_read_txt_file(filename: str, buffer_hint: int = -1, encoding="utf-8", errors=None, verbose=False):
    log_level = LOGGER.getEffectiveLevel()
    if verbose:
        LOGGER.setLevel(logging.DEBUG)
    # TODO refatorar todas as chamadas para ler arquivos compactados para desconderar a primeira linha se estiver nessa condição: line.startswith("/") and line.endswith("''")
    ...
    with open_file(filename, **kwargs) as opened_file:
        ...
        async def _gen_():
            ...
            while lines:
                ...
                await asyncio.sleep(diff_seconds * 0.1)  # to allow the event loop to take control
